...
signed main() {
	...
	ll[1]=rr[n]=1;
	for(int i = 2; i <= n; ++i)
		ll[i]=(ll[i-1]&((b[i] == 0 || b[i-1] == 0) ? 1 : (b[i] <= k/b[i-1]/min(b[i], b[i-1]))));//考试时少了个/min(x,y),似了
	for(int i = n-1; i >= 1; --i)
		rr[i]=(rr[i+1]&((b[i] == 0 || b[i+1] == 0) ? 1 : (b[i] <= k/b[i+1]/min(b[i], b[i+1]))));//考试时少了个/min(x,y),似了
	if(ll[n] == 0) {
		puts("0");
		return 0;
	}
	...
	for(int i = 1; i <= n; ++i) 
		if(a[i] == -1) {
			...
			LL ls = (i == 1 || a[i-1] == 0 ? k : (a[i-1] > tt ? ef(a[i-1]) : k/a[i-1]/a[i-1])), 
			   rs = (i == n/*考试时判成i+1 == n了,似了*/ || a[i+1] == 0 ? k : (a[i+1] > tt ? ef(a[i+1]) : k/a[i+1]/a[i+1]));
