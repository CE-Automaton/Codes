...
using namespace std;
const int N = 1e5+5;
int n, m, u, v, dep[N], siz[N], mxson[N], dfn[N], top[N], don[N], cnt, opt, up[N], tot, zhan[N];
vector <int> to[N];
struct pnt {
...
struct tree {
    ...
    node ask(int k, int l, int r, int lq, int rq) {
        // cout<<l<<" "<<r<<" "<<lq<<" "<<rq<<" "<<tr[k].col<<" "<<tr[k].pd<<" "<<tr[k].sum[0]<<"\n";
        if(lq <= l && r <= rq)
            return tr[k];
        int mid = ((l+r)>>1);
        down(k, l, mid, r);
    ...
    for(int i = 1; i <= m; ++i) {
        ...
        if(opt == 1) {
            ...
            for(int j = tot; j >= 1; --j) {
                int xx = zhan[j];
                if(up[top[xx]])
                    A.updtt(1, 1, n, dfn[up[top[xx]]], -A.ask(1, 1, n, dfn[top[xx]], dfn[don[top[xx]]]).sum[0]);
            }
            for(int j = tot; j >= 1; --j) {
                int xx = zhan[j];
                A.updt(1, 1, n, dfn[top[xx]], dfn[xx]);
            }
            for(int j = 1; j <= tot; ++j) {
                int xx = zhan[j];
                if(up[top[xx]])
            ...
            if(ner == 0) {
                ...
                // cout<<t[2].col<<" "<<t[dfn[5]].col<<" "<<t[1].sum<<"!?\n";
            }
            else {
                // A.updtt(1, 1, n, dfn[ner], 0);
                // cout<<ner<<"??? "<<don[top[ner]]<<" "<<t[ner].sum<<" "<<A.ask(1, 1, n, dfn[ner]+1, dfn[don[top[ner]]]).sum[0]<<"!?\n";
                // cout<<t[2].col<<" "<<t[3].col<<" "<<t[4].col<<" "<<t[5].col<<" "<<t[6].col<<"!?\n";
                // cout<<t[2].sum<<" "<<t[3].sum<<" "<<t[4].sum<<" "<<t[5].sum<<" "<<t[6].sum<<"!?\n";
                ans+=t[dfn[ner]].sum+1;
        ...
        // cout<<A.ask(1, 1, n, 1, 8).sum[0]<<" "<<A.ask(1, 1, n, 1, 8).col<<" "<<A.ask(1, 1, n, 1, 8).pd<<"\n";
        // cout<<A.ask(1, 1, n, 3, 3).sum[0]<<"? "<<A.ask(1, 1, n, 4, 4).sum[0]<<"? "<<A.ask(1, 1, n, 5, 5).sum[0]<<"?...\n";
        // cout<<A.ask(1, 1, n, 3, 4).col<<", "<<A.ask(1, 1, n, 3, 4).pd<<", "<<A.ask(1, 1, n, 3, 8).sum[0]<<", "<<A.ask(1, 1, n, 5, 8).sum[0]<<"!...\n\n";
    }
	return 0;
}