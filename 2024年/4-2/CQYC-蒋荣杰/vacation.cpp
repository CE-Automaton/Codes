...
const int N = 3e5+5;
int t, n, m, u, v, ans, zhan[N], pd[N], in[N], inn[N], dfn[N], low[N], sum[N], ll[N], pp, op, nr[N], yn[N], bbb[N];
LL w, dis[N], disn[N];
struct qwq {
	int x; LL ww;
...
void sol() {
	...
	for(int i = 1; i <= n; ++i)
		...
		dis[i]=inf, disn[i]=inf, dfn[i]=low[i]=in[i]=inn[i]=sum[i]=nr[i]=0, yn[i]=1;
	op=0;
	for(int i = 1; i <= m; ++i)
	...
	for(int x = 1; x <= n; ++x) 
		for(int i = 0; i < to[x].size(); ++i) {
			...
			if(dis[x]+disn[y]+to[x][i].w == dis[n]+1) {
				// debug(x, y, sum[nr[x]], sum[nr[y]], sum[yn[x]], sum[yn[y]], "!?!?!?!?!?!?\n");
				...
				if(abs(sum[nr[x]]-sum[yn[y]])) {
					if(sum[nr[x]] > sum[yn[y]])
						++bbb[sum[yn[y]]], --bbb[sum[nr[x]]];
					else
						++bbb[sum[nr[x]]], --bbb[sum[yn[y]]];
				}
				if(abs(sum[yn[x]]-sum[nr[y]])) {
					if(sum[yn[x]] > sum[nr[y]])
						++bbb[sum[nr[y]]], --bbb[sum[yn[x]]];
					else
						++bbb[sum[yn[x]]], --bbb[sum[nr[y]]];
				}
				if(abs(sum[yn[x]]-sum[yn[y]])) {
					if(sum[yn[x]] > sum[yn[y]])
						++bbb[sum[yn[y]]], --bbb[sum[yn[x]]];
					else
						++bbb[sum[yn[x]]], --bbb[sum[yn[y]]];
				}
			}
		}
	// for(int i = 1; i <= pp; ++i) 
