...
signed main() {
	...
	for(int i = 1; i <= q; ++i) 
		xx[b[i].d]=i;
	// for(int i = 1; i <= q; ++i) {
	// 	auto it = qqq.lower_bound(pii {xx[i], yy[i]});
	// 	if(it != qqq.end()) {
	// 		if((*it).second != yy[i])
	// 			rr[i]=(*it).first;
	// 		if(it != qqq.begin()) {
	...
	// 	}
	// 	if(!qqq.empty()) {
	// 		if((*qqq.rbegin()).second != yy[i] && (*qqq.rbegin()).first >= xx[i])
	// 			lll[i]=(*qqq.rbegin()).first;
	// 		if((*qqq.begin()).second != yy[i] && (*qqq.begin()).first <= xx[i])
	// 			rrr[i]=(*qqq.begin()).first;
	// 	}
	// 	qqq.insert(pii {xx[i], yy[i]});
	// }
	nd[0]=node {0, 0, 0};
	for(int i = 1; i <= q; ++i)
	...
	for(int i = 1; i <= q; ++i) {
		...
		if(b[mx[yy[i]]].w-b[mn[yy[i]]].w >= L && (mmx[yy[i]] == inf || mmn[yy[i]] == inf || (mmx[yy[i]] != inf && mmn[yy[i]] && 1LL*k*L+b[mmn[yy[i]]].w-b[mmx[yy[i]]].w >= L)))
			pd|=1;
		// debug(pd, "???\n");
		if(pd) {
			putchar('0');
