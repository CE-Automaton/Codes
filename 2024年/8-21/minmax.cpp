...
const int N = 5e3+5;
int n, a[N], b[N], dp[2][N], c[N][N], f[N], jc[N];
...
signed main() {
	...
	for(int l = 1; l <= n; ++l) {
		...
		a[l]=upd(upd(a[l]+mod)+mod);//mod没有1e9，要加两次！！！
		now^=1;
		for(int j = 0; j <= n; ++j)
			dp[now][j]=0, f[j]=0;
	...
	for(int l = 1; l <= n; ++l) {
		...
		b[l]=upd(upd(b[l]+mod)+mod);//mod没有1e9，要加两次！！！
		now^=1;
		for(int j = 0; j <= n; ++j)
			dp[now][j]=0, f[j]=0;
		for(int j = 0; j+r-l <= n; ++j)
		...
		int sum = 0;
		for(int j = n; j >= 0; --j)
			sum=upd(sum+f[j]), dp[now][j]=upd(dp[now][j]+1LL*(r-l+1)*sum%mod*b[l]%mod), sum=1LL*sum*b[l]%mod;
		l=r;
	}
	write(dp[now][0], ' ');
	return 0;
