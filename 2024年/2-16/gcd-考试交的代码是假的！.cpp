...
int n, a[N], st[N][LG], ll[N][LG], mnl[N], mnr[N], rr[N][LG], totl[N], totr[N], ls[N][LG], rs[N][LG];
...
signed main() {
	...
	for(int i = 1; i <= n; ++i)	{
		...
		for(int j = 2; 1LL*j*j <= ll[i][totl[i]]; ++j)
			if(ll[i][totl[i]]%j == 0) {
				mnl[i]=j;
				break;
			}
		...
		// debug('\n');
		for(int j = 2; 1LL*j*j <= rr[i][totr[i]]; ++j)
			if(rr[i][totr[i]]%j == 0) {
				mnr[i]=j;
				break;
			}
	...
	for(int i = 1; i <= n; ++i) {
		...
		if(i > 1 && i < n && a[i-1] != 1 && a[i+1] != 1) {
			// debug(mnl[i-1], mnr[i+1], gcd(mnl[i-1], mnr[i+1]), '\n');
			LL ab = 1LL*mnl[i-1]*mnr[i+1]/gcd(mnl[i-1], mnr[i+1]);//两边最小gcd值的最小质因子的最小公倍数
			if(ab <= 500000)
